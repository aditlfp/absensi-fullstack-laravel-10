			...
			<div class="w-full flex flex-col">
				<div class="mb-2">
					<button id="modalPulangBtn"
						class="bg-green-600 flex justify-center shadow-md hover:bg-green-700 text-white hover:shadow-none px-3 py-1 text-xl rounded-md transition all ease-out duration-100 mt-5 mr-0 sm:mr-2 uppercase items-center"><span
							class="font-bold" onClick="openModal()">Create New</span>
					</button>
				</div>
                        ...
                        <div class="flex justify-center flex-col text-xs ml-4 mr-5">
                            ...
                            <div class="flex w-3/6 mb-2">
                                <div>
                                    ...
                                    <input type="text" id="client_id" name="client_id" class="input input-bordered input-xs"
                                        value="{{ Auth::user()->kerjasama->client->id }}" readonly hidden>
                                    <input type="text" class="input input-bordered input-xs" value="{{ Auth::user()->kerjasama->client->name }}"
                                        readonly>
                            ...
                            <div class="flex w-3/6 mb-2">
                                ...
                                <div>
                                    ...
                                    @if ($pagi)
                                        <input type="text" id="shift" name="shift" class="input input-bordered input-xs" value="Shift Pagi"
                                            readonly>
                                    @elseif($siang)
                                        <input type="text" id="shift" name="shift" class="input input-bordered input-xs" value="Shift Siang"
                                            readonly>
                                    @else
                                        <input type="text" id="shift" name="shift" class="input input-bordered input-xs" value="Shift Malam"
                                            readonly>
                                    @endif
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$(document).on('click', '#modalPulangBtn', function() {
				$('.modalp').removeClass('hidden').addClass('flex opacity-100'); // Add opacity class

                $.ajax({
                type: 'GET',
                url: "{{ route('brief.create') }}",
                success: function(data) {
                    console.log(data);
                    $("#page").html(data);
                    $('.modalp').removeClass('hidden').addClass('flex opacity-100');
                    $('#head-modal').html('Create Briefing');
                    }
                })
			});
		...
		$(document).ready(function() {
			...
			$('#ajax-crud-datatable').DataTable({
				...
				columns: [{
					...
					{
						data: 'materi_breafing',
						name: 'materi_breafing'
					},
					{
						data: 'created_at',
						name: 'created_at'
		...
		$('#EmployeeForm').submit(function(e) {
            ...
            let id = $('#id').val();
			$.ajax({
				type: id?'PATCH':'POST', // Use PUT for updates and POST for creates
