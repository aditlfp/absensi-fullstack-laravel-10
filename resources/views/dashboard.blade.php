...
<body class="font-sans antialiased  bg-slate-400">
	<div class="min-h-screen pb-[12.5rem]">
					...
					<div class="py-5">
						...
						<div class="flex flex-col items-center gap-2 justify-center pt-2 px-2 overflow-hidden">
							...
							<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="isiAbsen">
								<a href="historyAbsensi" class="btn btn-info w-full">Riwayat Absensi</a>
							</div>
						...
						@if (Auth::user()->divisi->jabatan->code_jabatan == 'MITRA')
							<div class="flex flex-col items-center gap-2 justify-center pt-2 px-2 overflow-hidden">
								...
								<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="Labsensi">
									<a href="{{ route('lead_absensi') }}" class="btn btn-info w-full">Absensi,
										{{ Auth::user()->kerjasama->client->name }}</a>
								</div>
								<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="Llaporan">
									<a href="{{ route('lead_laporan') }}" class="btn btn-info w-full">Laporan,
										{{ Auth::user()->kerjasama->client->name }}</a>
								</div>
								<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="Llembur">
									<a href="{{ route('lead_lembur') }}" class="btn btn-info w-full">Lembur,
										{{ Auth::user()->kerjasama->client->name }}</a>
								</div>
								<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="lizin">
									<a href="{{ route('lead_izin') }}" class="btn btn-info w-full">Izin,
										{{ Auth::user()->kerjasama->client->name }}</a>
								</div>
								<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="Luser">
									<a href="{{ route('lead_user') }}" class="btn btn-info w-full">User,
										{{ Auth::user()->kerjasama->client->name }}</a>
								</div>
								<div class="hidden w-full space-y-4 px-2 sm:px-16 overflow-hidden" id="Ljadwal">
									<a href="{{ route('leader-jadwal.index') }}" class="btn btn-info w-full">Jadwal User,
										{{ Auth::user()->kerjasama->client->name }}</a>
								</div>
							</div>
						@endif
		...
		@if (Carbon\Carbon::now()->format('Y-m-d') >= '2023-08-16' && Carbon\Carbon::now()->format('Y-m-d') <= '2023-08-18')
			<div
				class="bg-[url('/public/logo/indoCuy.jpg')] bg-center bg-cover mx-10 my-5 shadow-md p-4 rounded w-fit flex items-center flex-col">
				<img src="https://www.animatedimages.org/data/media/781/animated-indonesia-flag-image-0015.png"
					class="border-none w-20 p-2 rounded" alt="">
				<p class="font-semibold indent-2 text-center bg-white p-1 rounded">Selamat Hari Kemerdekaan indonesia yang ke
					{{ Carbon\Carbon::now()->format('Y') - '1945' }} </p>
			</div>
		@else
		@endif
